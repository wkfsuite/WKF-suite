...
if (typeof Chart === 'undefined') {
  document.body.innerHTML = '<div style="text-align:center;padding:50px;color:#e74c3c;">Error: Chart.js not loaded. Check your internet connection.</div>';
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (!['admin', 'supervisore', 'segreteria'].includes(ruolo)) {
      ...
      showToast(`Access denied for role: ${ruolo}. Analytics available only for Admin, Supervisor and Secretariat`, 'error');
...
async function loadAllAnalytics() {
  ...
  try {
    ...
    const chartPromises = [
      loadMonthlyChart().catch(err => console.error('Monthly chart error:', err)),
      loadTypeChart().catch(err => console.error('Type chart error:', err)),
      loadEmployeesChart().catch(err => console.error('Employees chart error:', err)),
      loadHoursChart().catch(err => console.error('Hours chart error:', err))
    ];
    ...
    showToast('Partial loading error', 'warning');
  } finally {
...
async function loadMonthlyChart() {
  try {
    ...
    document.getElementById('monthlyChart').parentNode.innerHTML =
      '<div class="error">Error loading monthly chart</div>';
  }
}
...
async function loadTypeChart() {
  try {
    ...
    document.getElementById('typeChart').parentNode.innerHTML =
      '<div class="error">Error loading type chart</div>';
  }
}
...
async function loadEmployeesChart() {
  try {
    ...
    console.error('Error loading employees chart:', error);
    document.getElementById('employeesChart').parentNode.innerHTML =
      '<div class="error">Error loading employees chart</div>';
  }
}
...
async function loadHoursChart() {
  try {
    ...
    document.getElementById('hoursChart').parentNode.innerHTML =
      '<div class="error">Error loading hours chart</div>';
  }
}
...
function setupUpgradeButtons() {
  ...
  if (upgradeBtn) {
    upgradeBtn.addEventListener('click', () => {
      if (window.StripeUpgrade) {
        ...
        showToast('❌ Upgrade system unavailable. Try again later.', 'error');
      }
    });
  }
  ...
  if (activateLicenseBtn) {
    activateLicenseBtn.addEventListener('click', async () => {
      ...
      if (!licenseKey) {
        licenseMsg.innerHTML = '<div class="message error">Enter a valid license key</div>';
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          licenseMsg.innerHTML = '<div class="message success">License key activated successfully!</div>';
          setTimeout(() => {
            licenseModal.style.display = 'none';
            window.location.reload();
          }, 2000);
        } else {
          licenseMsg.innerHTML = `<div class="message error">${result.message || 'Invalid license key'}</div>`;
        }
      } catch (error) {
        console.error('License activation error:', error);
        licenseMsg.innerHTML = '<div class="message error">Error during license activation</div>';
      }
    });
  }
