...
async function caricaTutteLeRichieste(ruolo) {
  ...
  if (!container) {
    console.error(`Container non trovato per ruolo: ${ruolo}`);
    return;
  }
    ...
    richieste.forEach(r => {
      ...
      div.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr auto; gap: 15px; align-items: start;">
          <div>
            <strong>Dipendente:</strong> ${r.nome || r.matricola}<br>
            ...
            <strong>Richiesta il:</strong> ${new Date(r.createdAt).toLocaleDateString('it-IT')}<br>
...
async function approvaRichiesta(richiestaId) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert('Errore: ' + (error.error || 'Impossibile approvare la richiesta'));
    }
  } catch (err) {
    console.error(err);
...
async function rifiutaRichiesta(richiestaId) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const error = await res.json();
      alert('Errore: ' + (error.error || 'Impossibile rifiutare la richiesta'));
    }
  } catch (err) {
    console.error(err);
    alert('Errore di connessione');
  ...
  if (segreteriaForm) {
    segreteriaForm.addEventListener("submit", async (e) => {
      ...
      if (!matricolaDipendente || !tipo) {
        showMsg("Matricola dipendente e tipo permesso sono obbligatori");
        return;
      }
      ...
      if (tipo === "giornaliero") {
        ...
        const data = document.getElementById("dataOreSegreteria").value;
      ...
      try {
        ...
        if (res.ok) {
          showMsg("Richiesta inserita correttamente per il dipendente!", "green");
          ...
          showMsg(result.error || "Errore inserimento richiesta");
        }
      } catch (err) {
        console.error(err);
  ...
  if (img) {
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <h2>🌐 Sistema Gestione Permessi - QR Code</h2>
          <div class="qr-print">
  ...
  modal.innerHTML = `
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button class="btn secondary" onclick="this.closest('.modal-overlay').remove()">Annulla</button>
      </div>
    </div>
  `;
...
function mostraLicenseKeyModal() {
  ...
  document.getElementById('activateLicenseBtn').onclick = async () => {
    ...
    if (!licenseKey) {
      msgDiv.innerHTML = '<div class="message error">Inserisci una license key valida</div>';
      return;
    }
...
async function checkAnalyticsAccess() {
  try {
    ...
    if (licenseData.isPro) {
      ...
      showToast('📊 Analytics Avanzate disponibili solo con licenza PRO!', 'warning');
      mostraUpgradeModal();
    }
  } catch (error) {
