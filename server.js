...
app.get('/api/richieste/:id/pdf', requireAuth, (req, res) => {
  ...
  db.get("SELECT * FROM richieste WHERE id=?", [req.params.id], (err, row) => {
    ...
    db.get("SELECT * FROM company_settings WHERE id = 1", (errSettings, settings) => {
      ...
      const dataDisplay = row.data || (row.dal && row.al ? `${row.dal} to ${row.al}` : "-");
      const oreDisplay = row.oraInizio && row.oraFine ? `${row.oraInizio} - ${row.oraFine}` : "-";

      // Determine the type from the record
      let tipologiaDisplay = "💰 Paid"; // Default
      if (row.daRecuperare) tipologiaDisplay = "⏰ To Be Recovered";
      else if (row.nonRetribuito) tipologiaDisplay = "❌ Unpaid";
      else if (row.permessoSindacale) tipologiaDisplay = "🏛️ Union Leave";
      else if (row.inCFerie) tipologiaDisplay = "🏖️ Vacation";
    ...
    res.send(`
      ...
      <body>
        <div class="header">
          ...
          <div class="document-title">LEAVE REQUEST</div>
        </div>
        <div class="info"><strong>Name:</strong> ${row.nome || '-'} ${row.cognome || ''}</div>
        <div class="info"><strong>Employee ID:</strong> ${row.matricola}</div>
        <div class="info"><strong>Leave Type:</strong> ${row.tipo}</div>
        <div class="info"><strong>Category:</strong> ${tipologiaDisplay}</div>
        <div class="info"><strong>Date/Period:</strong> ${dataDisplay}</div>
        <div class="info"><strong>Schedule:</strong> ${oreDisplay}</div>
        <div class="info"><strong>Notes:</strong> ${row.note || '-'}</div>
        ...
        <div class="footer">
          <p>Generated by Leave Management System - ${new Date().toLocaleString('en-US')}</p>
        </div>
        ...
        <div class="ecological-message no-print" style="
          ...
          font-size: 12px;
        ">
          <div style="font-weight: bold; margin-bottom: 8px;">🌍 DON'T PRINT - SAVE THE PLANET</div>
        ...
        <div class="no-print" style="margin-top: 30px; text-align: center;">
          <p style="color: #999; font-size: 14px; margin-bottom: 10px;">💡 Use Ctrl+P to print or save as PDF</p>
          <button onclick="window.close()" style="padding: 10px 20px; font-size: 16px; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer;">✖️ Close</button>
        </div>
      </body>
      </html>
      `);
